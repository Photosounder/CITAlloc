...
This implementation replaces the allocator when compiling to native Windows binaries. It works by reserving one large section of virtual memory (currently 256 GB) and progressively commiting more of it as needed, with this buffer being treated as the whole heap. A mutex is used so that allocations can be done in parallel.
