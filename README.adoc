...
This implementation replaces the allocator when compiling to native Windows binaries. It works by reserving one large section of virtual memory (1 TB or the maximum available in 32-bit mode) and progressively commiting more of it as needed, with this buffer being treated as the whole heap. I rounded up the base address after a few days of being tired of looking at addresses that look like `0x1a8808c0680`, it's still random but round. A mutex is used so that allocations can be done in parallel.
