...
This is a malloc/free/realloc replacement meant mainly for WebAssembly modules but adaptable to any situation, I'm also starting to use it in my Windows programs. The goal is to have an allocator that stores information about all allocated buffers in a compact table that can easily be read and analysed, in my main case from the module's host. This is achieved by having a table that is treated just like any other dynamically allocated buffer and each entry shows which entries come before and after it, where the buffer starts and ends, when a buffer was created or last modified as well as extra info such as the name of the function, file and line number that the call originates from. All this information is meant to provide clarity so that we can clearly see what's going on in our code and use those insights to use dynamic allocation in more sensible ways.

CIT Alloc also reports helpful errors with a maximum level of detail, for instance attempting to free() or realloc() an address that is in the middle of an existing buffer will report not only the illegal call but also report inside which buffer that address is found, complete with information about the line of code that allocated that buffer in the first place. A `malloc(0);` will also be reported so you can be aware that this happens but proceeds as if it was `malloc(1);`. On Windows I use a message box to report such errors because `stderr` is easily ignored. If you think about it it really sucks that regular implementations don't report anything, like it can know you're doing something wrong but it doesn't give you a way to find out.

CIT Alloc can also optionally erase all the unused bytes in the given memory space, so for instance when I use it in my WebAssembly modules all the unused bytes in the heap are always set to `0xC5`. This helps keep a very clean heap that makes more sense when looking at it and also can help catch bugs, because if you see a variable set to `0xC5C5C5C5` you know that means it was uninitialised, and if you look at the visualiser and see data in an empty space that might indicate use after free or a buffer overflow. I like to have this enabled by default at all times. A padding between all buffers can also be added, and because of free space cleaning this can make buffer overruns easy to see.

Besides the compact information table a second information buffer called the map may also exist (it's optional but used by default). This map contains cells that contain the index of a buffer found in a 8 kB range (adjustable with the `CITA_MAP_SCALE` define) so that when we look for an address we can start from that buffer and look for it in either direction. When we look for a buffer's index by its address (or an address inside of it) we quickly (by a `-` and a `>>`) find the index of its cell in the map which gives us the index of a buffer in the table that serves as a starting point from which we can look for the buffer that encompasses the address we're looking for, so in the worst case we only look through 8 kB worth of allocations instead of always starting from the beginning which would be many MB or even GB worth of allocations. Map cells can also optionally contain a second number that indicates the size of the largest free memory chunk going through the cell to help with finding free spaces.

Information in the table is customisable: indices can be 8, 16 or 32-bit integers (16-bit indices will allow for about 65,532 allocations as index 0 is the base, 1 is the table, 2 may be the optional map and `0xFFFF` is "not an index"), an optional pair of indices points to the previous and next gap in memory to help finding free spaces at allocation, the creation and modification timestamps can be excluded, the link flag depends on whether link checking is done and the info string's length can be adjusted, even down to zero. So with 16-bit indices and 32-bit addresses we can store as little as 12 bytes per allocation. In fact if we used this allocator on something like a microcontroller with less than 64 kB of RAM we could use 8-bit indices and 16-bit addresses so each allocation would take 6 bytes.
...
This implementation replaces the allocator when compiling to native Windows binaries. It works by reserving one large section of virtual memory (1 TB or the maximum available in 32-bit mode, can be changed by defining `CITA_WIN_MAX`) and progressively commiting more of it as needed, with this buffer being treated as the whole heap. I rounded up the base address after a few days of being tired of looking at addresses that look like `0x1a8808c0680`, it's still random but round, it's rounded up so that only two hexadecimal digits remain. A mutex is used so that allocations can be done in parallel.
...
int main(int argc, char *argv[])
{
	char *ptr = malloc(1<<30);
	printf("CITA buffer at %#zx, buffer size %zd, malloc returned %#zx (offset %#zx)\n", (size_t) cita_buffer.mem, (cita_buffer.mem_end-(size_t) cita_buffer.mem), (size_t) ptr, (size_t) ptr-(size_t) cita_buffer.mem);
...
This prints:
```
CITA buffer at 0x20000000000, buffer size 1073745920, malloc returned 0x200000005e0 (offset 0x5e0)
CITA buffer at 0x20000000000, buffer size 1073745920, malloc returned 0x200400005e0 (offset 0x400005e0)
```

=== cita_mac.h

Similar to cita_windows.h. Base addresses aren't rounded up.
